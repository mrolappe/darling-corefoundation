  ...
  if (size > 0)
    {
      ...
      if (mprotect(buffer, getpagesize(), PROT_READ|PROT_WRITE) == -1)
	{
	  // NSLog(@"Failed to protect memory as writable: %@", [NSError _last]);
	}
#endif
      NSDeallocateMemoryPages(buffer, getpagesize());
#endif
#endif
      buffer = 0;
      executable = 0;
...
+ (id) allocWithZone: (NSZone*)aZone
{
  if (self == NSInvocation_abstract_class)
    {
      return [NSInvocation_concrete_class alloc];
    }
  else
    {
...
- (NSString*) description
{
  ...
  NSString* str = (NSString*) CFStringCreateWithCString(NULL, buffer);
...
{
  GSInvocationProxy	*o;
  o = (GSInvocationProxy*) [self alloc];
  o->target = RETAIN(t);
  return o;
}
- (NSInvocation*) _invocation
