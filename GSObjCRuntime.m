...
GSObjCMethodNames(id obj, BOOL recurse)
{
  ...
  while (class != Nil)
      ...
      while (count-- > 0)
	{
	  ...
	  name = CFStringCreateWithCString(NULL, sel_getName(method_getName(meth[count])));
	  [set addObject: (NSString*) name];
	  CFRelease(name);
	}
      if (meth != NULL)
	{
...
GSObjCVariableNames(id obj, BOOL recurse)
{
  ...
  while (class != Nil)
      ...
      while (count-- > 0)
	{
	  ...
	  name = CFStringCreateWithCString(NULL, ivar_getName(ivar[count]));
	  [set addObject: (NSString*) name];
	  CFRelease(name);
	}
      if (ivar != NULL)
