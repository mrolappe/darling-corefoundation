...
- (NSUInteger)replaceOccurrencesOfString:(NSString *)target withString:(NSString *)replacement options:(NSStringCompareOptions)options range:(NSRange)searchRange
{
	...
	{
		// FIXME: This doesn't make sense because the superclass is __NSCFType 
		//return [super replaceOccurrencesOfString:target withString:replacement options:options range:searchRange];
		printf("__NSCFString replaceOccurrencesOfString:withString:options:range: doesn't handle regex yet");
		return 0;
	}
	else
	{
...
- (const char *)cStringUsingEncoding:(NSStringEncoding)encoding
{
	...
	{
		// FIXME: __NSCFType doesn't respond to cStringUsingEncoding
		printf("__NSCFType doesn't respond to cStringUsingEncoding");
...
static inline const char *cStringForEncoding(__NSCFString *self, CFStringEncoding encoding, BOOL allowLossyConversion)
{
	...
	if (str == NULL)
	{
		...
		// FIXME
		printf("getBytes not available in __NSCFString");
