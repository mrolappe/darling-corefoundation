...
// Neither of these classes are actually in CoreFoundation

// They are in Foundation
...
static BOOL _rangeCheckRange(__NSCFAttributedString *self, NSRange range)
{
	...
	{
		[NSException raise:NSRangeException format:@"range (%lu,%lu) beyond NSAttributedString bounds (%lu)",
			(unsigned long)range.location, (unsigned long)range.length, (unsigned long)[self length]];
		return NO;
	}
	return YES;
}
...
static BOOL _rangeCheckIndex(__NSCFAttributedString *self, NSUInteger index)
{
	...
	{
		[NSException raise:NSRangeException format:@"index (%lu) beyond NSAttributedString bounds (%lu)",
